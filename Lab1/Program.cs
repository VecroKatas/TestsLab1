...
while (x0 == 0)
{
    ...
    if (_x0 >= a && _x0 <= b)
    {
        if (Func(_x0) * F2(_x0) > 0)
        {
            x0 = _x0;
            
            Console.WriteLine("f(x_0) = " + Func(x0) + "; f``(x_0) = " + F2(x0));
        }
        else
        {
            Console.WriteLine("Wrong x_0 input.");
        }
        Console.WriteLine("f(x_0) * f``(x_0) > 0 = " + (Func(_x0) * F2(_x0) > 0));
    }
    else
...
Console.WriteLine("n \t\t x_n \t\t f(x_n)");
Console.WriteLine("0 \t\t" + xi + "\t\t" + Func(xi));

for (int i = 1; i <= n0; i++)
{
    xi = xi - Func(xi) / F1(xi);
    Console.WriteLine(i + "\t\t" + xi + "\t\t" + Func(xi));
}
Console.WriteLine();
...
while (MathF.Abs(x_ - xi) > MathF.Pow(10, -e))
{
    ...
    xi = xi - Func(xi) / F1(xi);
    j++;
    Console.WriteLine(j + "\t\t" + xi + "\t\t" + Func(xi));
}
